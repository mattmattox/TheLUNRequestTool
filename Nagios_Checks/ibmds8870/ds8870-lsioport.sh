...
########################################################################################
#
#       Filename: ds8870-lsioport.sh
#       Author: Matthew Mattox matt@mattox.work
#       Date: 02/03/2015
#       Purpose: Nagios Service Check to verify IO Port status
...
then
	echo "##############################"
	echo "RAW DATA"
	/opt/ibm/dscli/dscli -cfg $CFG "lsioport -fmt delim -hdr off -dev $DEVID"
	echo "##############################"
fi
####Get array status - start
STATUS=$(/opt/ibm/dscli/dscli -cfg $CFG "lsioport -fmt delim -hdr off -dev $DEVID" | grep -v "Online")
####Get array status - end

if [[ -z "$STATUS" ]];
then
	echo "OK: All IO Ports are Online"
        exit 0
else
	echo "CRITICAL: Problem found with IO Port" $(echo $STATUS | grep -v "Online" | awk -F"," '{print $1}')
        exit 2
fi
