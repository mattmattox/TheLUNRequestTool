...
then
	echo "##############################"
	echo "RAW DATA"
	/opt/ibm/dscli/dscli -cfg $CFG "lsrank -fmt delim -hdr off -dev $DEVID"
	echo "##############################"
fi
####Get array status - start
STATUS=$(/opt/ibm/dscli/dscli -cfg $CFG "lsrank -fmt delim -hdr off -dev $DEVID" | grep -v "Normal")
####Get array status - end

if [[ -z "$STATUS" ]];
then
	echo "OK: All Ranks are Online"
        exit 0
else
	echo "CRITICAL: Problem found with Rank" $(echo $STATUS | grep -v "Normal" | awk -F"," '{print $1}')
        exit 2
fi
