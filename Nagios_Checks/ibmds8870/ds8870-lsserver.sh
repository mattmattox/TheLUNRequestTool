...
then
	echo "##############################"
	echo "RAW DATA"
	/opt/ibm/dscli/dscli -cfg $CFG "lsserver -fmt delim -hdr off"
	echo "##############################"
fi
####Get array status - start
STATUS=$(/opt/ibm/dscli/dscli -cfg $CFG "lsserver -fmt delim -hdr off" | grep -v ",online")
####Get array status - end

if [[ -z "$STATUS" ]];
then
        echo "OK: All Servers are Normal"
        exit 0
else
        echo "CRITICAL: Problem found with Server" $(echo $STATUS | grep -v ",online" | awk -F"," '{print $1}')
        exit 2
fi
